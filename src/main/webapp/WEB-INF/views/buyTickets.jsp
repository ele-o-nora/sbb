...
<c:if test="${!empty ticketOrder}">
    <form:form modelAttribute="ticketOrder" method="post" action="${pageContext.request.contextPath}/finalizeTicketSale">
...
<c:if test="${!empty transferTickets}">
    <form:form modelAttribute="transferTickets" method="post" action="${pageContext.request.contextPath}/finalizeTicketSale">
        <c:if test="${!empty passenger}">
            <div class="row m-2">
                First name: <span class="text-danger">${passenger.firstName}</span>
                Last name: <span class="text-danger">${passenger.lastName}</span>
        ...
        <div class="row justify-content-center m-2">
            <div class="col-sm-6 m-1 border">
                ...
                <form:input type="hidden" path="firstTrain.journey.id" value="${transferTickets.firstTrain.journey.id}"/>
                <form:input type="hidden" path="firstTrain.origin.id" value="${transferTickets.firstTrain.origin.id}"/>
                <form:input type="hidden" path="firstTrain.destination.id" value="${transferTickets.firstTrain.destination.id}"/>
            ...
            <div class="col-sm-6 m-1 border">
                ...
                <form:input type="hidden" path="secondTrain.journey.id" value="${transferTickets.secondTrain.journey.id}"/>
                <form:input type="hidden" path="secondTrain.origin.id" value="${transferTickets.secondTrain.origin.id}"/>
                <form:input type="hidden" path="secondTrain.destination.id" value="${transferTickets.secondTrain.destination.id}"/>
